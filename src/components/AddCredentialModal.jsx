...
export default function AddCredentialModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🔑 ZKP Step: generate nonce and commitment
      const nonce = generateNonce();
      localStorage.setItem("zkNonce", nonce);
      ...
      // generate response using nonce
      const response = generateResponse(challenge, nonce);

      // verify credential on-chain
      await contract.verifyCredential(response, ipfsHash);
